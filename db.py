...
def init_db() -> tuple[bool, str]:
    """Create the database and the rayalty table if they don't exist."""
    try:
        # First, ensure the database itself exists on the PostgreSQL server
        from urllib.parse import urlparse, urlunparse
        parsed = urlparse(DB_URL)
        target_db = parsed.path.lstrip('/')
        if target_db:
            postgres_url = urlunparse(parsed._replace(path='/postgres'))
            conn_temp = psycopg2.connect(postgres_url)
            conn_temp.autocommit = True
            cur_temp = conn_temp.cursor()
            cur_temp.execute("SELECT 1 FROM pg_database WHERE datname = %s", (target_db,))
            if not cur_temp.fetchone():
                cur_temp.execute(f'CREATE DATABASE "{target_db}"')
            cur_temp.close()
            conn_temp.close()
    except Exception as e:
        # Log error or continue to main connection attempt (which will fail with a clear message)
        pass
