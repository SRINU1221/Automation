...
def _should_use_manual_creds(plant_name: str, mode: str) -> bool:
    pn = plant_name.strip().lower()
    swap_plants = {"byathole", "girmapur", "madharam210", "nalthur", "rachaloor", "siddapur"}
    all_manual_plants = {"mamidipally"}
    if pn in all_manual_plants:
        return True
    if pn in swap_plants:
        return mode != "TP"
    return mode == "TP"
...
with _col_l:
    ...
    st.markdown('<div class="card"><div class="card-t">⚙️ Run Configuration</div>',
                unsafe_allow_html=True)
    _cfg["mode"] = st.selectbox("Mode", ["TP", "MDL", "Govt. Royalty"],
                                 index=["TP","MDL","Govt. Royalty"].index(_cfg.get("mode","TP")),
    ...
    # ── Credentials card — behaviour depends on selected mode and plant custom rules ─
    _use_manual = _should_use_manual_creds(_sel, _cfg["mode"])

    # Initialise TP credential store for this plant if missing (acts as the manual input cache)
    ...
    if _use_manual:
        # User supplies username + password manually
        st.markdown(f"""
<div class="card">
  <div class="card-t">🔐 {_cfg["mode"]} Login Credentials</div>
  ...
  border-radius:10px;padding:10px 14px;margin-bottom:12px;font-size:.8rem;color:#fbbf24;">
    ✏️ <b>{_cfg["mode"]} mode</b> — enter your login username &amp; password below.
    These are used only for this run and are never stored.
        ...
        _tp_creds["username"] = st.text_input(
            ...
            key=f"tp_user_{_sel}",
            disabled=_rt["running"],
            placeholder=f"Enter {_cfg['mode']} username…"
        )
        _tp_creds["password"] = st.text_input(
            ...
            type="password",
            disabled=_rt["running"],
            placeholder=f"Enter {_cfg['mode']} password…"
        )
        st.markdown('</div>', unsafe_allow_html=True)
    else:
        # Credentials come silently from database
        st.markdown(f'<div class="card"><div class="card-t">🔒 Credentials (from Database - {_cfg["mode"]} mode)</div>',
                    unsafe_allow_html=True)
        st.markdown(f"""
...
<div style="margin-top:6px;font-size:.75rem;color:#475569;">
  🔒 Username &amp; password auto-loaded from DB · To update go to <b>⚙️ Plant Config</b>
    ...
    # Start / Stop
    # Validate that the user has entered credentials if manual input is enabled
    _use_manual = _should_use_manual_creds(_sel, _cfg["mode"])
    _creds_ok = (not _use_manual) or (
        bool(_tp_creds.get("username", "").strip()) and
        bool(_tp_creds.get("password", "").strip())
    )
    _ready = bool(_cfg.get("records")) and _creds_ok
    if _use_manual and not _creds_ok and _cfg.get("records"):
        st.warning(f"⚠️ Enter your {_cfg['mode']} portal username and password above before starting.")
    ...
    with _b1:
        if st.button(f"🚀 Start {_sel}", disabled=_rt["running"] or not _ready,
            ...
            # Use manually entered credentials if _use_manual is True, otherwise read from DB
            if _use_manual:
